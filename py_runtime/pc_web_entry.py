...
def apply_settings(settings: dict[str, Any] | None = None) -> dict[str, Any]:
    # Apply settings coming from Web UI.
    # IMPORTANT: settings can be a delta (partial). Never reset fields that are not present.
    # Accept both camelCase and snake_case keys for backwards/forwards compatibility.
    from PIL import Image
    ...
    def _has(*keys: str) -> bool:
        return any(k in settings_obj for k in keys)
            ...
            if not enabled_dyes:
                ...
                try:
                    for dye in list_dyes():
                        if isinstance(dye, dict) and isinstance(dye.get('id'), int):
                            fallback_ids.append(int(dye['id']))
                except Exception:
                    fallback_ids = []
    ...
    # --- BEST COLORS (optional) ---
    if _has('bestColors', 'best_colors'):
        best_colors = _get('bestColors', 'best_colors')
        try:
    ...
    if _has('borderConfig', 'border_config', 'border'):
        ...
        if isinstance(border_raw, dict):
            ...
            if 'frame_image' not in border_raw and 'frameImage' in border_raw:
                border_raw = dict(border_raw)
                border_raw['frame_image'] = border_raw.get('frameImage')
        border_config = _normalize_border_config(border_raw)
        ...
        def _load_frame_rgba(path: Path) -> 'Image.Image':
            cache = globals().get('_FRAME_IMAGE_CACHE', None)
            cache_max = int(globals().get('_FRAME_IMAGE_CACHE_MAX', 0) or 0)
            debug = bool(globals().get('_BORDER_FRAME_DEBUG', False))

            key = str(path)
            if isinstance(cache, dict) and cache_max > 0:
                img = cache.get(key)
                if img is not None:
                    try:
            ...
            if debug and '_BORDER_FRAME_OPEN_COUNT' in globals():
                try:
                    globals()['_BORDER_FRAME_OPEN_COUNT'] = int(globals().get('_BORDER_FRAME_OPEN_COUNT', 0)) + 1
                except Exception:
                    pass
            ...
            if isinstance(cache, dict) and cache_max > 0:
                ...
                try:
                    while len(cache) > cache_max:
                        cache.popitem(last=False)  # type: ignore[arg-type]
                except Exception:
                    pass
            return img
        ...
        if border_config['style'] == 'image' and frame_image_path is not None:
            if frame_image_key != current_frame_key:
                frame_img = _load_frame_rgba(frame_image_path)
                if bool(globals().get('_BORDER_FRAME_STRICT', False)):
                    original_set_image = getattr(controller, 'set_image', None)
                    try:
                        def _blocked_set_image(*_args: Any, **_kwargs: Any) -> None:
                            raise AssertionError('set_image() must not be used for border frame images')
                        if callable(original_set_image):
                            controller.set_image = _blocked_set_image  # type: ignore[assignment]
                        controller.set_border_frame_image(frame_img)
                    finally:
                        if callable(original_set_image):
                            controller.set_image = original_set_image  # type: ignore[assignment]
                else:
                    controller.set_border_frame_image(frame_img)
                setattr(state, 'border_frame_image_key', frame_image_key)
        else:
            if current_frame_key is not None:
                controller.set_border_frame_image(None)
                setattr(state, 'border_frame_image_key', None)

    # --- PREVIEW MODE ---
    if _has('preview_mode', 'previewMode'):
        preview_mode = str(_get('preview_mode', 'previewMode') or '').strip().lower()
    ...
    if _has('canvasRequest', 'canvas_request'):
        set_canvas_request(_get('canvasRequest', 'canvas_request'))
